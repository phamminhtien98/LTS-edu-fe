...
const ThanhVienChua = ({ listPhatTu }) => {
    ...
    return (
        <>
            <div className='title'><TeamOutlined />  Danh sách phật tử của chùa:</div>
            <Input.Search placeholder='Nhập tên hoặc pháp danh'
                style={{ maxWidth: "400px", margin: "10px 0px" }}
                onSearch={(value) => { setSearch(value) }}
                onChange={(e) => { setSearch(e.target.value) }} />
            <Table columns={columns} dataSource={listPhatTu}
                pagination={{
...
export default memo(ThanhVienChua) 